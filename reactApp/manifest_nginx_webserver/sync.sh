...
argocd app create nginxapp --repo 'https://github.com/ablaamim/reactApp.git' --path 'reactApp/manifest' --dest-namespace 'applications' --dest-server 'https://kubernetes.default.svc' --grpc-web

sleep 5

# Retrieve details of the newly created application
argocd app get nginxapp --grpc-web
# Sleep for 5 seconds to prevent potential connection issues
sleep 5

# Synchronize the application state with the git repository
argocd app sync nginxapp --grpc-web
# Sleep for 5 seconds to prevent potential connection issues
sleep 5

# Set the application to automatically synchronize changes
argocd app set nginxapp --sync-policy automated --grpc-web
# Sleep for 5 seconds to prevent potential connection issues
sleep 5

# Set the application to automatically prune resources that are removed from the git repository
argocd app set nginxapp --auto-prune --allow-empty --grpc-web
# Sleep for 5 seconds to prevent potential connection issues
sleep 5
